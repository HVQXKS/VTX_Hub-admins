    ...
    local SelectedToy = ToyList["Mic"]  -- ← Microfone como padrão

    -- Dropdown + Toggle
    box:AddDropdown("AntiInputLagToy", {
        Text = "Input Lag Item",
        Values = dropdownValues,
        Default = table.find(dropdownValues, "Mic") or 1,
    ...
    box:AddToggle("AntiInput", {
        ...
        Callback = function(Value)
            ...
            task.spawn(function()
                ...
                local SelectedToyName = SelectedToy
                ...
                while _G.AntiInputLag do
                    ...
                    local toy = toysFolder:FindFirstChild(SelectedToyName)
                    
                    if not toy then
                        pcall(function()
                            SpawnRemote:InvokeServer(SelectedToyName, hrp.CFrame * CFrame.new(0, 2, 0), Vector3.zero)
                        end)
                        RunService.Heartbeat:Wait()
                        continue
                    end
                    ...
                    if holdPart then
                        ...
                        if holdingPlayer and holdingPlayer ~= plr then
                            pcall(function()
                                holdPart.DropItemRemoteFunction:InvokeServer(toy, hrp.CFrame * CFrame.new(0, 1000, 0), Vector3.zero)
                            end)
                            pcall(toy.Destroy, toy)
                        else
                            -- Loop rápido de Hold + Drop (Anti Input Lag)
                            while _G.AntiInputLag and toy and toy.Parent do
                                pcall(function()
                                    holdPart.HoldItemRemoteFunction:InvokeServer(toy, char)
                                    holdPart.DropItemRemoteFunction:InvokeServer(toy, hrp.CFrame * CFrame.new(0, 3000, 0), Vector3.zero)
                                end)
                                if not toy or not toy.Parent or not toysFolder:FindFirstChild(SelectedToyName) then
                                    break
                                end
                                RunService.Heartbeat:Wait()
                            end
                        ...
                        -- Se não tiver HoldPart, recria
                        pcall(function()
                            if toy and toy.Parent then toy:Destroy() end
                            SpawnRemote:InvokeServer(SelectedToyName, hrp.CFrame * CFrame.new(0, 3, 0), Vector3.zero)
                        end)
                        RunService.Heartbeat:Wait()
                    end
                end
    ...
    print("✅ Anti Input Lag com Microfone carregado!")
        ...
        box:AddLabel("TP To Mouse"):AddKeyPicker("TPTOMOUSE", {
            ...
            Callback = function()
                if Mouse.Target then
                    local targetCFrame = Mouse.Hit * CFrame.new(1, 4, 1)
                            ...
                            if Method == "Loop Grab(Kick)" then
                                ...
                                if Sets.HRP and hum and HRP then
                                    ...
                                    if not kickbp or kickbp.Parent ~= Sets.HRP then
                                        ...
                                        kickbp.D = 200
                                        kickbp.Position = HRP.Position + Vector3.new(4.5, 13.5, 0)
                                    end
            ...
            Callback = function(on)
                ...
                task.spawn(function()
                    ...
                    while Toggles.LoopKickBlob.Value do
                        ...
                        if tRoot and tHum and tHum.Health > 50 and blobRoot then
                            blobRoot.CFrame = SavedPos
                            tRoot.CFrame = SavedPos * CFrame.new(3, 24, 0)

                            if tick() - packetTimer > 0.012 then
                                
                                packetTimer = 0.012 tick()
                                pcall(function()
        ...
        box:AddToggle("RemoveAntiInputLagTarget", {
            ...
            Callback = function(state)
                ...
                task.spawn(function()
                    ...
                    Library:Notify("Anti Input Lag on: " .. target.DisplayName, 5)
                    ...
                    local function StartMonitoring()
                        ...
                        if folder then
                            ...
                            connections[#connections+1] = folder.ChildAdded:Connect(function(item)
                                task.wait(0.05) -- pequeno delay para o item carregar
                                ProcessItem(item)
                            end)
                        end
                    end
    ...
    local GucciAtivado = false
    local loopConnection = nil

    -- ==================== BOTÃO PARA ATIVAR ====================
    box:AddButton("Activate remove gucci", function()
        if GucciAtivado then 
            print("Remove Gucci it's already activated") 
            return 
        end
        
        GucciAtivado = true
        print("Remove gucci on")

        loopConnection = task.spawn(function()
            while GucciAtivado do
                local Seat = Seats[Sets.Name]
                
                if Seat and hum and HRP then
                    -- Força sentar
                    Seat:Sit(hum)
                    
                    -- Reseta velocidade e posição
                    task.wait(0.03)
                    stvel(HRP)
                    HRP.CFrame = HRP.CFrame  -- mantém a posição atual
                end
                
                task.wait(0.15) -- Quanto menor, mais forte (mas pode dar lag)
            end
        end)
    end)

    -- ==================== BOTÃO PARA DESATIVAR ====================
    box:AddButton("Disable remove gucci", function()
        if not GucciAtivado then 
            print("Remove gucci already deactivated") 
            return 
        end
        
        GucciAtivado = false
        print("Remove gucci off")
